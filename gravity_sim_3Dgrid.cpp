...
int main() {
 ...
 while (!glfwWindowShouldClose(window) && running == true) {
    ...
    // Mass adjustment code...
    if (!objs.empty() && objs.back().Initalizing) {
        // ... existing mass adjustment code ...
    }

    // === PHYSICS UPDATE LOOP (separate from rendering) ===
    if (!pause) {
        for(auto& obj : objs) {
            if (!obj.Initalizing) {
                // Calculate forces from other objects
                for(auto& obj2 : objs) {
                    if(&obj2 != &obj && !obj2.Initalizing) {
                        ...
                        if (distance > 0.1f) {
                            ...
                            float minDistance = (obj.radius + obj2.radius) * 10.0f;
    ...
    // === RENDERING LOOP (only drawing, no physics) ===
    // Draw the grid
glUseProgram(shaderProgram);
glUniform4f(objectColorLoc, 1.0f, 1.0f, 1.0f, 0.5f); // Increase alpha to 0.5 for more visibility
glUniform1i(glGetUniformLocation(shaderProgram, "isGrid"), 1);
glUniform1i(glGetUniformLocation(shaderProgram, "GLOW"), 0);

// Regenerate grid vertices every frame
std::vector<float> gridVertices = CreateGridVertices(10000.0f, 50, objs);
glBindBuffer(GL_ARRAY_BUFFER, gridVBO);
glBufferData(GL_ARRAY_BUFFER, gridVertices.size() * sizeof(float), gridVertices.data(), GL_DYNAMIC_DRAW);

// Debug: Print grid vertex count
static int frameCount = 0;
if (frameCount % 60 == 0) {
    std::cout << "Grid vertices: " << gridVertices.size() << std::endl;
}
frameCount++;

// Draw the grid with proper settings
glDisable(GL_DEPTH_TEST); // Draw grid without depth testing so it's always visible
DrawGrid(shaderProgram, gridVAO, gridVertices.size());
glEnable(GL_DEPTH_TEST);  // Re-enable for objects

// Draw objects AFTER grid
